...
export class ChatBlock extends Component {
  ...
  render() {
    ...
    const messages = chat && chat.messages.map((message) => {
      ...
      const regExp = '^(sticker[1-8])$';
      if (message.text.match(regExp)) {
        message.sticker = (Object.values(stickers)[message.text[7] - 1]);
      } else {
  ...
  didMount() {
    ...
    Object.entries(stickers).forEach((sticker, index) => {
      stickerPicker.innerHTML += `<img class="chat__sticker-picker_sticker" data-link="sticker${index + 1}" src="${sticker[1]}" >`;
    });
